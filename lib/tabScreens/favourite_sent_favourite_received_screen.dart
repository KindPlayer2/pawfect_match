...
class _FavouriteSentFavououriteReceivedScreenState extends State<FavouriteSentFavououriteReceivedScreen> 
{
  ...
  getFavoriteListKeys() async
  {
    ...
    {
      var favoriteSentDocument = await FirebaseFirestore.instance.collection("users").doc(currentUserID.toString()).collection("favoriteSent").get();
    ...
    {
      var favoriteRecievedDocument = await FirebaseFirestore.instance.collection("users").doc(currentUserID.toString()).collection("favoriteReceived").get();
                ...
                child: GestureDetector(
                  ...
                  child: DecoratedBox(
                    decoration: BoxDecoration(
                      image: DecorationImage(image: NetworkImage(favouriteList[index]["imageProfile"],),
                      fit: BoxFit.cover,
                      )
                    ),
                    child: Padding(
                      ...
                      child: Center(
                        child: Column(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                favouriteList[index]["name"].toString() + " • " + favouriteList[index]["age"].toString(),
                            ...
                            Expanded(
                              child: Text(
                                favouriteList[index]["city"].toString() + " • " + favouriteList[index]["country"].toString(),
                                style: const TextStyle(
                                  overflow: TextOverflow.ellipsis,
                                  color: Colors.white,
                                  fontSize: 16,
